...
async function registerAgent(did, capability) {
...
async function publishTask(description, budget = 10) {
...
async function bid(taskId, agentDid, amount) {
...
async function settle(taskId) {
  const res = await fetch(`${COORD_URL}/v1/tasks/${taskId}/settle`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(COORD_API_KEY ? { 'x-api-key': COORD_API_KEY } : {}),
